...
這份文件說明如何在 Claude 相關環境中啟用本專案使用的 MCP servers。
...
若任務需要 NotebookLM，另外還應確保：

- MCP server 名稱：`nblm-mcp-server`
- MCP URL：`http://localhost:8089/mcp`
- `nblm-mcp-server` 已先在本機正常啟動
- NotebookLM 驗證狀態已就緒（例如 `~/.notebooklm/profiles/default/storage_state.json` 可用；舊版環境可能仍使用 `~/.notebooklm/storage_state.json`）
...
2. 若已有設定檔，依既有格式加入需要的 MCP server
3. 若沒有可參考範例，再請用戶提供目前使用的 Claude 設定方式

## 建議

若目前只是想快速驗證 MCP server 能不能用，而不想先處理 Claude 的環境差異，可優先改用 `mcporter`：
